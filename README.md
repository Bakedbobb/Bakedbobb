# Hello there 👋 I'm Matt!
![Discord](https://img.shields.io/badge/Discord-bakedbobb-5865F2?style=for-the-badge&logo=discord) ![Badge](https://hitscounter.dev/api/hit?url=https%3A%2F%2Fgithub.com%2Fbakedbobb&label=Visitors&icon=github&color=%230a58ca&message=&style=for-the-badge&tz=Poland)

I’m a self-taught software developer from Poland 🇵🇱. I’ve been coding since 2020, building personal projects with a strong focus on open source and code quality. 

When I’m not studying or programming, I usually watch movies, mainly from the last century.

### Experience
`2024 – Present`
- Founder & Lead Developer ▸ [Moonage-Studios](https://github.com/Moonage-Studios)
- Founder ▸ [MoonageMC](https://github.com/MoonageMC)

### Stack
- **Web** · JavaScript · Tailwind CSS · HTML<br>
- **Backend** · Java · PaperAPI · Maven · Gradle · Python · PHP · Laravel*<br>
- **Storage** · MySQL · SQLite · PostgreSQL* · Redis*<br>
###### _* Currently learning_

### Weekly development breakdown
<!--START_SECTION:waka-->

```java
From: 20 August 2026 - To: 27 August 2026

Total Time: 16 mins

Shell Script      9 mins                ▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▱▱▱▱▱▱▱▱▱▱   58.46 %
Java Properties   6 mins                ▰▰▰▰▰▰▰▰▰▰▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱   38.34 %
GitIgnore file    0 secs                ▰▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱   02.86 %
Markdown          0 secs                ▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱   00.34 %
```

<!--END_SECTION:waka-->
